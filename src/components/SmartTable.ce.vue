<template>
    <div class="smart-table-main" :style="`font-family: ${fontFamily}`">
            ...
            <div class="smart-table-export-hub-dropdown" :data-is-open="isExportRequest || 'false'">
                ...
                <div class="smart-table-export-hub-options">
                    <div class="smart-table-export-hub-option-wrapper">
                        <div class="label">
                            {{ parsedTranslations.exportAllLabel }} 
                            ({{ bodyCopy.length }})
                        </div>
                        <button id="exportAll" @click="createXls('all')">
                            <div v-html="icons.fileDownload"/>
                            <span>{{ parsedTranslations.exportAllButton }}</span>
                        </button>
                    </div>
                    <div class="smart-table-export-hub-option-wrapper">
                        <div class="label">
                            {{ parsedTranslations.exportPageLabel }}
                        </div>
                        <button id="exportPage" @click="createXls('page')">
                            <div v-html="icons.fileDownload"/>
                            <span>{{ parsedTranslations.exportPageButton }}</span>
                        </button>
                    </div>
                </div>
            </div>
                        ...
                        <tr>
                            ...
                            >
                                <div class="th-filter">
                                    ...
                                    <input
                                        v-if="th.isSearch"
                                        :placeholder="parsedTranslations.inputPlaceholder"
                                        v-model="searches[i]"
            ...
            <template v-if="currentSelectionSpan.col !== undefined && currentSelectionSpan.rows.length">
                ...
                <span>
                    ...
                    <span style="margin-left:12px">
                        {{ parsedTranslations.nb }} : <b class="format-num">{{ currentSelectionSpan.rows.length }}</b>
                    </span>
        ...
        <div class="smart-table-paginator format-num" v-if="bodyCopy.length > 10">
            {{ parsedTranslations.totalRows }} : {{ JSON.parse(body).length }} |  
            {{ parsedTranslations.paginatorLabel }} :
            <select 
            ...
            >
                <template v-for="(option, i) in paginatorOptions">
                    <option 
                        ...
                        v-if="bodyCopy.length > option || JSON.parse(body).length === option"
                    >
                        {{ option }}
                    </option>
                </template>
        ...
        <div class="smart-table-size-warning" v-if="itemsPerPage >= 250">
            <span v-html="icons.warning"/>{{ parsedTranslations.sizeWarning }}
        </div>
                        ...
                        <g v-for="(plot, i) in chartData.plots" :key="`plot_${i}`">
                            ...
                            <template v-if="chart.type === constants.BAR">
                                ...
                                <rect 
                                    ...
                                    :y="plot.y" 
                                    :height="chart.height - plot.y" 
                                    :width="chartData.slot" 
                                    :fill="selectedPlot === i ? 'url(#barGradientSelected)' : 'url(#barGradient)'" 
                                    stroke="white" 
                                    stroke-width="1"
                                />
                                <foreignObject 
...
export default {
    ...
    data() {
        return {
            bodyCopy: JSON.parse(this.body).map((el, i) => {
                return {
                    ...el,
                    absoluteIndex: i,
                }
            ...
            multiselects: {},
            paginatorOptions: [...new Set([10, 25, 50, 100, 250, 500, this.rowsPerPage, JSON.parse(this.body).length])].sort((a,b) => a - b),
            palette: [
                '#3366cc',
                '#dc3912',
                '#ff9900',
            ...
            sorts: {},
            tableBody: JSON.parse(this.body).map((el, i) => {
                return {
                    ...el,
                    absoluteIndex: i
                }
            }),
            tableHead: JSON.parse(this.header).map(head => {
                return {
    ...
    methods: {
        ...
        closeAllDropdowns() {
            const dropdowns = this.$el.querySelectorAll(".th-dropdown");
        ...
        getDatesRange(index) {
            const dates = JSON.parse(this.body).map(row => new Date(row.td[index]));
        ...
        navigateCell(event) {
            ...
            const nextCellRow = this.$el.querySelector(`#cell_${currentRow}_${currentCol + 1}`);
            const previousCellRow = this.$el.querySelector(`#cell_${currentRow}_${currentCol - 1}`);
            const nextCellCol = this.$el.querySelector(`#cell_${currentRow + 1}_${currentCol}`);
            const previousCellCol = this.$el.querySelector(`#cell_${currentRow - 1}_${currentCol}`);
                ...
                this.tableHead.forEach((th,i) => {
                    ...
                    if(th.rangeFilter) {
                        Object.assign(this.rangeFilters, {[i]: {
                            min: Math.round(Math.min(...JSON.parse(this.body).map(el => el.td).map(el => el[i]))),
                            max: Math.round(Math.max(...JSON.parse(this.body).map(el => el.td).map(el => el[i]))),
                        }});
                    ...
                    if(th.type === this.constants.NUMERIC && !th.isPercentage) {
                        Object.assign(this.hasNaN, {[i]: this.includesNaN(JSON.parse(this.body).map(el => Number(el.td[i])))})
                    }
                });
        ...
        resetSelection() {
            const highlightedCells = this.$el.querySelectorAll(`.${this.cssClass.ROW}`);
            ...
            if (this.currentSelectionSpan.col !== index) {
                this.visibleRows.forEach((row, i) => {
                    this.selectTd({
                        ...
                        event: {
                            currentTarget: this.$el.querySelector(`#cell_${i}_${index}`)
                        }
                    });
                });                
        ...
        toggleMultiselect(index, _th, event) {
            ...
            const menu = this.$el.querySelector(`#th_dropdown_${index}`);
